...
class PaymentTransactionParcialRefund(models.TransientModel):
    ...
    payment_id = fields.Many2one(
        "payment.transaction",
        string="Payment",
        domain=[("state", "=", "done"), ("acquirer_id.provider", "=", "decidir_v2")],
        required=True,
    )
...
class PaymentTransactionSpsRefund(models.Model):
    ...
    payment_id = fields.Many2one("payment.transaction", string="Payment", required=True)
    amount = fields.Float(string="Amount", required=True)
...
class PaymentTransaction(models.Model):
    ...
    sps_payment_instalment = fields.Integer(string="Decidir instalment")
    ...
    def set_decidir_data(self, sps_send):
        ...
        if "status_details" in sps_send:
            ...
            self.sps_card_authorization_code = sps_send["status_details"][
                "card_authorization_code"
            ]
            self.sps_address_validation_code = sps_send["status_details"][
    ...
    def decidir_get_payment_info(self):
        ...
        for transaction in self.filtered(
            lambda t: t.acquirer_id.provider == "decidir_v2"
        ):
            ...
            if "only_show_data" in self.env.context:
                ...
                if transaction_info["status"] == "annulled" and transaction.state in [
                    "draft",
                    "authorized",
                    "done",
                ]:
    ...
    def decidir_refunds_partial_payment(self, amount):
        for transaction in self:
            ...
            new_transation = {
                ...
                "sps_card_authorization_code": transaction_info["status_details"][
                    "card_authorization_code"
                ],
                "sps_address_validation_code": transaction_info["status_details"][
                    "address_validation_code"
                ],
            }
            ...
            transaction.payment_id.amount = (
                transaction.amount + transaction.fees + refund_amount
            )
    ...
    def payment_decidir_send_payment(self, token, card_bin):
        self.reference = "TX%s-%s" % (self.id, datetime.now().strftime("%y%m%d_%H%M%S"))
        payload = {
