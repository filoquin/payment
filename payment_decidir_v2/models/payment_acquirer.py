...
class PaymentAcquirer(models.Model):
    ...
    def decidir_cancel_refund(self, payment_id, refund_id):
        ...
        # merchantId
        api_url = self.decidir_get_base_url() + "/payments/%i/refunds/%i" % (
            payment_id,
            refund_id,
        )
    ...
    def decidir_refund_payment(self, payment_id, amount=0):
        ...
        # merchantId
        api_url = (
            self.decidir_get_base_url() + "/payments/" + str(payment_id) + "/refunds"
        )
        headers = self.decidir_get_headers()
        payload = "{}"
        if amount:
    ...
    def decidir_get_payments(
        self, dateFrom=False, dateTo=False, siteOperationId=False, offset=0
    ):
    ...
    def payment_decidir_make_token(
        self, card_id, end_string, partner_id, add_token, bin
    ):
        token = {
    ...
    def payment_decidir_create_payment(
        self, order, token, amount, fees, instalment, card_bin
    ):
        values = {
            "amount": amount,
            "fees": fees,
    ...
    def decidir2_instalment_tree(self, order=False, order_id=False, amount_total=0):
        ...
        if order:
            ...
            if not amount_total:
                amount_total = (
                    self.env["sale.order"].sudo().browse(order_id).amount_total
                )

        # to-do  los instaments debeen venir desde el payment (self)
        instalment_ids = (
            self.env["account.card.instalment"]
            .sudo()
            .search([("website_enabled", "=", True)])
        )

        cards = {}
        for card in instalment_ids.mapped("card_id"):
            cards[card.id] = {"name": card.name, "id": card.id, "instalments": []}

        tree = {}
        if amount_total:
            for instalment in instalment_ids:
                if instalment.card_id.id not in tree:
                    tree[instalment.card_id.id] = cards[instalment.card_id.id]
