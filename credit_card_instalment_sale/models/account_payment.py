...
class AccountPayment(models.Model):
    ...
    account_card_ids = fields.Many2many(
        "account.card", string="Cards", related="journal_id.account_card_ids"
    )
    card_id = fields.Many2one("account.card", string="Card")

    instalment_id = fields.Many2one("account.card.instalment", string="Instalment plan")

    instalment = fields.Integer(
        string="instalment plan", related="instalment_id.instalment"
    )
    ...
    def compute_clearing_date(self):
        for payment in self:
            if len(payment.instalment_id) and len(
                payment.instalment_id.payment_term_id
            ):
                payment.clearing_date = payment.instalment_id.payment_term_id.compute(
    ...
    def _prepare_payment_moves(self):
        ...
        for payment in self:
            ...
            if len(payment.instalment_id) and len(
                ...
                moves_vals["line_ids"][0][
                    "date_maturity"
                ] = payment.instalment_id.payment_term_id.compute(
                    payment.amount, payment.payment_date
                )[
                    0
                ][
    ...
    def action_register_sale_payment(self):
        ...
        payment_amout = sum(
            sale_order.payment_ids.filtered(
        ...
        for rec in self:
            if rec.fee != 0.0:
                ...
                if len(rec.draft_invoice_ids):
                    ...
                    fee_invoice_id.invoice_line_ids = [
                        (
                            0,
                            0,
                            {
            ...
            if rec.fee != 0.0:
                if len(rec.sale_order_id):
                    if rec.instalment_id.product_id.taxes_id:
                        tax_amount = rec.instalment_id.product_id.taxes_id.compute_all(
                            rec.fee
                        )
                        ...
                        if tax_amount["total_included"] > rec.fee:
                            tax_amount = (
                                rec.instalment_id.product_id.taxes_id.with_context(
                    ...
                    line_vals.append(
                        {
                            ...
                            "tax_id": [
                                (6, 0, rec.instalment_id.product_id.taxes_id.ids)
                            ],
                            "sequence": 999,
                        }
                    )
                    ...
                    if rec.discount_amount:
                        ...
                        if rec.instalment_id.product_id.taxes_id:
                            tax_amount = (
                                rec.instalment_id.product_id.taxes_id.compute_all(
                                    rec.discount_amount
                                )
                            )
                            if tax_amount["total_included"] > rec.discount_amount:
                                tax_amount = (
                                    rec.instalment_id.product_id.taxes_id.with_context(
                                        force_price_include=True
                                    ).compute_all(rec.discount_amount)
                                )
                        ...
                        line_vals.append(
                            {
                                "order_id": rec.sale_order_id.id,
                                "product_id": rec.instalment_id.product_id.id,
                                "name": _("discount off %% %s")
                                % rec.instalment_id.discount,
                                "product_uom_qty": 1,
                    ...
                    if rec.instalment_id.bank_discount:
                        bank_discount = amount * (
                            (100 + rec.instalment_id.bank_discount) / 100
                        )
                        line_vals.append(
                            {
                                "order_id": rec.sale_order_id.id,
                                "display_type": "line_note",
                                "name": _("bank discount off %% %s %s")
                                % (rec.instalment_id.bank_discount, bank_discount),
                                "is_financial_line": True,
                                "sequence": 999,
                            }
                        )
                        ...
                        fee_invoice_id = self.env["account.move"].create(
                            {
                                ...
                                "invoice_user_id": rec.invoice_ids[
                                    0
                                ].invoice_user_id.id,
            ...
            if len(record.instalment_id):
                if record.instalment_id.coefficient:
                    ...
                    if record.instalment_id.product_id.taxes_id:
                        ...
                        tax_amount = (
                            record.instalment_id.product_id.taxes_id.compute_all(amount)
                        )
