...
class SaleOrder(models.Model):
    ...
    def _compute_amount(self):
        for so in self:
            ...
            payment_amout = sum(
                so.payment_ids.filtered(
                    lambda x: x.state not in ["draft", "cancelled"]
                    and x.payment_type_copy != "outbound"
                ).mapped("amount")
            )
            credit_note_amout = sum(
                so.payment_ids.filtered(
                    lambda x: x.state not in ["draft", "cancelled"]
                    and x.payment_type_copy == "outbound"
                ).mapped("amount")
    ...
    def action_register_sale_payment(self):
        ...
        if self.amount_residual > 1.0:
            return (
                ...
                .with_context(
                    active_ids=self.ids, active_model="sale.order", active_id=self.id
                )
                .action_register_sale_payment()
            )
        elif self.invoice_status == "to invoice":
            if self.amount_residual != 0:
                round_product_id = self.env["ir.config_parameter"].get_param(
                    "round_product_id", False
                )
                if round_product_id:
                    round_product = self.env["product.product"].browse(
                        int(round_product_id)
                    )
                    self.order_line = [
                        (
                            0,
        ...
        if "order_line" not in default:
            default["order_line"] = [
                ...
                for line in self.order_line.filtered(
                    lambda l: not l.is_downpayment and not l.is_financial_line
                )
            ]
        else:
            default["order_line"] = [
                (0, 0, line)
                for line in default["order_line"].filtered(
                    lambda l: not l.is_downpayment and not l.is_financial_line
                )
            ]
