...
class AccountCardInstalment(models.Model):
    ...
    amount = fields.Float(string="Fix amount")
    coefficient = fields.Float(
        string="coefficient", help="Value to multiply the amount", default=1.0
    )
    discount = fields.Float(string="discount", help="discount in amount")
    bank_discount = fields.Float(string="bank discount", help="Bank discount")
    active = fields.Boolean("Active", default=True)
    ctf = fields.Float(string="C.T.F.")
    tea = fields.Float(string="TEA")
    ...
    def _compute_name(self):
        for record in self:
            if record.name == "/" and len(record.card_id) and record.instalment:
                record.name = "%s-%s" % (record.card_id.name, record.instalment)
