...
class PosPaymentMethod(models.Model):
    ...
    def _get_payment_terminal_selection(self):
        return super(PosPaymentMethod, self)._get_payment_terminal_selection() + [('qr', 'QR')]
    ...
    pos_qr_image = fields.Binary(
        string='qr image',
        compute='_compute_pos_qr_image'
    )
    ...
    def _compute_pos_qr_image(self):
        for record in self:
            if record.qr_method == 'fix':
                ...
                if session_id:
                    qr_id = record.config_qr_ids.filtered(lambda x: x.config_id.id == session_id.config_id.id)
                    if qr_id:
    ...
    def check_qr_transaction(self, data):
        ...
        if len(method.acquirer_id):
            cust_method_name = '%s_pos_transaction_check' % (method.acquirer_id.provider)
            if hasattr(method.acquirer_id, cust_method_name):
                method = getattr(method.acquirer_id, cust_method_name)
                return method(data)
        return False            
    ...
    def set_qr_manual_authorization(self, data):
        ...
        data['config_id'] = self.env['pos.config'].browse(data['configId'])
        if len(method.acquirer_id):
            cust_method_name = '%s_set_qr_manual_authorization' % (method.acquirer_id.provider)
            if hasattr(method.acquirer_id, cust_method_name):
                method = getattr(method.acquirer_id, cust_method_name)
                return method(data)
        return False 
