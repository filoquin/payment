...
class PaymentLinkWizard(models.TransientModel):
    ...
    def _compute_is_qr_provider(self):
        for rec in self:
            if isinstance(rec.payment_provider_selection, int):
                provider = self.env['payment.provider'].sudo().browse(rec.payment_provider_selection)
    ...
    def _prepare_payment_transaction_vals(self):
        self.ensure_one()
        provider = self.env['payment.provider'].sudo().browse(int(self.payment_provider_selection))
        res = {
