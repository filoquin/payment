...
class CashControlConfig(models.Model):
    ...
    store_id = fields.Many2one("res.store", string="Store")
    ...
    sequence_id = fields.Many2one(
        "ir.sequence", string="Receipt Sequence", readonly=True, copy=False
    )
    current_session_id = fields.Many2one(
        "cash.control.session", string="Current Session", copy=False
    )
    ...
    is_acum_cash_control = fields.Boolean("Es Caja Acumuladora")
    is_main_cash_control = fields.Boolean("Es Caja Principal")
    ...
    last_balance_end_real = fields.Float(
        string="Last balance", compute="_compute_last_balance_end_real"
    )

    def _compute_last_balance_end_real(self):
        for config_id in self:
            if len(config_id.current_session_id):
                config_id.last_balance_end_real = (
                    config_id.current_session_id.statement_id.balance_end_real
                )
            else:
                ...
                config_id.last_balance_end_real = (
                    last_stmt and last_stmt.balance_end_real or 0
                )
    ...
    def write(self, values):
        if (
            not self.user_has_groups("ba_base.blancoamor_administacion")
            ...
            raise UserError(
                _('Only users with "%s" group can write a cash control config')
                % (group.name)
            )
    ...
    def open_session(self):
        ...
        transfer_id = self.env["cash.control.transfer.cash"].search(
            [("dest_cash_control_id", "=", self.id), ("state", "=", "transfer")]
        )
        if transfer_id:
    ...
    def check_user(self):
        if self.env.uid not in self.current_session_id.user_ids.ids:
            raise UserError(
                _("You are not the responsible of the cash control %s (%s)")
        ...
        for pos_config in self:
            ...
            if session:
                ...
                pos_config.last_session_closing_date = (
                    session[0]["date_end"].astimezone(timezone).date()
                )
                if session[0]["statement_id"]:
                    ...
                    pos_config.last_session_closing_cashbox = (
                        ...
                        .browse(session[0]["statement_id"][0])
                        .cashbox_end_id
                    )
                else:
                    pos_config.last_session_closing_cash = 0
