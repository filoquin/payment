...
class SaleOrder(models.Model):
    ...
    def invoice_and_conciled(self):
        ...
        if self.refund_move_lines_ids:
            ...
            invoice = self.env["account.move"].browse(
                report_action["context"]["active_ids"][0]
            )
    ...
    def _compute_amount(self):
        for so in self:
            ...
            refund_move_lines_amout = (
                sum(so.refund_move_lines_ids.mapped("amount_residual")) * -1
            )
            so.amount_residual = (
                so.amount_total
                - payment_amout
    ...
    def _create_invoices(self, grouped=False, final=False):
        ...
        if len(session):
            ...
            return super(
                SaleOrder,
                self.with_context(
                    default_journal_id=journal_id, cash_control_session_id=session.id
                ),
