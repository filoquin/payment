...
class SaleOrder(models.Model):
    ...
    instalment_count = fields.Integer(
        string="instalment count", compute="_compute_instalment_count"
    )
...
class SaleOrderLine(models.Model):
    ...
    price_1 = fields.Float(string="price 1", compute="compute_instalment")
    price_2 = fields.Float(string="price 2", compute="compute_instalment")
    price_3 = fields.Float(string="price 3", compute="compute_instalment")
    price_4 = fields.Float(string="price 4", compute="compute_instalment")
        ...
        for res in self:
            ...
            for i in range(0, 4):
                if len(res.order_id.instalment_ids) > i:
                    prices["price_%s" % str(i + 1)] = (
                        res.price_total * res.order_id.instalment_ids[i].coefficient
                    )
                else:
